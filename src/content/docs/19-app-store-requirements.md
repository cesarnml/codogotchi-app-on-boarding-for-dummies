---
title: "19 — App Store Distribution: What Would Have to Change"
---
...
Hold Chapter 09's producer/consumer split and Chapter 18's disk contract in
...
The whole contract lives in `~/.codogotchi` (Chapter 18). Inside the sandbox,
...
- **App Group container (recommended).** Move the contract directory to
  ...
  resolves through `CodogotchiFolders` / `CODOGOTCHI_HOME` (Ch.18), so the
...
rejection risk. The MAS build needs the demo mode (Ch.15's fixture driver) or
...
- **Network** (the gallery fetch, Convex) — just the `network.client`
  entitlement, routine.
- Reading/writing **inside** the group container — the entire Chapter 18
...
1. **See the container lie.** Sandbox any toy app (or read up on
   ...
   confirm it's the container path, not `/Users/you`. Now re-read Chapter 18
