...
After `tcis` ([`test-codogotchi-idle-set`](./15-dev-workflow-shell-helpers.md),
...
3. Can the preview-override channel (`tclb`/`tcle`) carry that number? Why or why
   not? (Re-read the **ghost exception**, [Ch.15](./15-dev-workflow-shell-helpers.md).)
...
(reuse the procedural-draw approach from [Ch.17](./17-procedural-effects-deep-dive.md),
